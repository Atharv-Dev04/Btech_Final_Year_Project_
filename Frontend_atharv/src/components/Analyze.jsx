...
import { DEMO_SENTIMENT, DEMO_SUMMARY, DEMO_KEYWORDS } from '../services/demoData';
...
export default function Analyze() {
    ...
    const handleAnalysisClick = async (doc) => {
        ...
        try {
            ...
            setAnalysisData({
                sentiment: sentiment.status === 'fulfilled' && sentiment.value?.status === 'success' ? sentiment.value.data : DEMO_SENTIMENT,
                summary: summary.status === 'fulfilled' && summary.value?.status === 'success' ? summary.value.data : DEMO_SUMMARY,
                keywords: keywords.status === 'fulfilled' && keywords.value?.status === 'success' ? keywords.value.data : DEMO_KEYWORDS
            });
        } catch (err) {
                                    ...
                                    <div className="space-y-12 animate-fade-in">
                                        ...
                                        <section className="space-y-6">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <h4 className="font-black text-white uppercase tracking-wider text-sm">Executive Summary</h4>
                                            </div>
                                            <div className="p-8 bg-indigo-500/5 rounded-[40px] border border-indigo-500/10 relative overflow-hidden group">
                                                ...
                                                <p className="text-lg text-indigo-50 font-medium leading-relaxed">
                                                    {analysisData.summary?.summary?.en || analysisData.summary?.summary || "No summary available for this intelligence audit."}
                                                </p>
                                        ...
                                        <section className="space-y-6">
                                            ...
                                            <div className="flex flex-wrap gap-3">
                                                {(analysisData.keywords?.keywords?.en || []).map((kw, i) => (
                                                    <motion.div
                                                        key={i}
