    ...
    const handleDocumentSelect = async (docId) => {
        ...
        try {
            ...
            if (response.status === 'success' && response.data.text) {
                ...
                // Fallback for demo IDs or incomplete response
                const demoDoc = DEMO_DOCUMENTS.find(d => d.document_id === docId);
                if (demoDoc) {
                    setSourceText(demoDoc.full_text || demoDoc.text_preview);
                    setTranslatedText('');
                    setStats(null);
                    setActiveTab('paste'); // Auto-switch to editor view
                    setSelectedDocId(''); // Reset selection after import
            ...
            console.error("Failed to load document content, trying demo fallback:", err);
            const demoDoc = DEMO_DOCUMENTS.find(d => d.document_id === docId);
            if (demoDoc) {
                setSourceText(demoDoc.full_text || demoDoc.text_preview);
                setTranslatedText('');
