...
import { DEMO_DISTRIBUTION, DEMO_TRENDS } from '../services/demoData';
    ...
    useEffect(() => {
        const fetchAnalytics = async () => {
            ...
            try {
                ...
                let distData = DEMO_DISTRIBUTION;
                if (distResponse.status === 'success' && (distResponse.data.positive || distResponse.data.neutral || distResponse.data.negative)) {
                ...
                setTotalReports(100);
                setSentimentData([
                    { name: 'Positive', value: DEMO_DISTRIBUTION.positive, color: '#10B981' },
                    { name: 'Neutral', value: DEMO_DISTRIBUTION.neutral, color: '#6366F1' },
                    { name: 'Negative', value: DEMO_DISTRIBUTION.negative, color: '#F43F5E' },
                ]);
                setSentimentTrendData(DEMO_TRENDS.map(item => ({
                    name: new Date(item.time).toLocaleDateString(undefined, { weekday: 'short' }),
                    positive: item.positive,
                    neutral: item.neutral,
                    negative: item.negative
        ...
        <div className="space-y-10 animate-fade-in max-w-6xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    ...
                    <p className="text-gray-400 text-lg font-medium">Linguistic emotional distribution across processed intelligence. <span className="text-indigo-400 text-xs font-black uppercase ml-2 tracking-tighter bg-indigo-500/10 px-2 py-0.5 rounded-md border border-indigo-500/20">Demo</span></p>
                </div>
